...
import categories from "../../pages/admin/data/collection.json";

function Collection() {
  const navigate = useNavigate();

  const handleCategoryClick = (categoryTitle) => {
    // Hero jaisa hi - navigate to products page with filter state
    navigate("/products", {
      state: {
    ...
    <section className="bg-[#fffaf5] py-20 overflow-hidden">
      <div className="text-center mb-12">
        <h2 className="text-3xl md:text-4xl font-light text-gray-800">
          Explore{" "}
          <span className="italic text-orange-600 font-serif">
            Festive Categories
          </span>
        </h2>
        <p className="text-gray-600 text-medium mt-2">
          Curated premium gifting collections
        </p>
      </div>
